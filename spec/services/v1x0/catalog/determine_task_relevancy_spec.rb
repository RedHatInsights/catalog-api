describe V1x0::Catalog::DetermineTaskRelevancy, :type => :service do
  ...
  let(:order_state_transition) { instance_double(V1x0::Catalog::OrderStateTransition) }
  ...
  describe "#process" do
    ...
    context "when the state is completed" do
      ...
      context "when the task context has a key path of [:service_instance][:id]" do
        ...
        before do
          allow(V1x0::Catalog::UpdateOrderItem).to receive(:new).and_return(update_order_item)
      ...
      context "when the task context has a key path of [:applied_inventories]" do
        ...
        before do
          allow(V1x0::Catalog::CreateApprovalRequest).to receive(:new).with(task).and_return(create_approval_request)
      ...
      context "when the task context does not have either key path" do
        ...
        context "when the status is 'error'" do
          ...
          before do
            allow(V1x0::Catalog::OrderStateTransition).to receive(:new).with(order_item.order.id).and_return(order_state_transition)
