RSpec.describe("v1.0 - GraphQL") do
  ...
  def result_portfolio_edges(response_body)
    JSON.parse(response_body).fetch_path("data", "portfolios", "edges")
  end
  ...
  context "different graphql queries" do
    ...
    it "querying portfolios return portfolio ids" do
      ...
      result_portfolio_ids = result_portfolio_edges(response.body).collect { |edge| edge.fetch_path("node", "id").to_i }
      expect(result_portfolio_ids).to match_array([portfolio_a.id, portfolio_b.id])
    end

    it "querying portfolios return portfolio names" do
      expect(response.status).to eq(200)
      result_portfolio_names = result_portfolio_edges(response.body).collect { |edge| edge.fetch_path("node", "name") }
