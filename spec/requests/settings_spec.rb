describe 'Settings API', :type => :request do
  ...
  context "when the user is a catalog admin" do
    before { allow(ManageIQ::API::Common::RBAC::Roles).to receive(:assigned_role?).with("Catalog Administrator").and_return(true) }
  ...
  context "when the user is not a catalog admin" do
    before { allow(ManageIQ::API::Common::RBAC::Roles).to receive(:assigned_role?).with("Catalog Administrator").and_return(false) }
