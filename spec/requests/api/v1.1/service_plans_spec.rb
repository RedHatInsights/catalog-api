describe "v1.1 - ServicePlansRequests", :type => [:request, :v1x1, :topology] do
  ...
  describe "#orderable" do
    context "when the schema has not changed" do
      it "returns a 200" do
        get "#{api_version}/service_plans/#{service_plan.id}/orderable", :headers => default_headers

        expect(response).to have_http_status :ok
      end
    end

    context "when the schema has changed" do
      let(:service_plan) { create(:service_plan) }
