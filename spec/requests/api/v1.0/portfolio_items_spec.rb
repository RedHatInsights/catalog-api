describe "v1.0 - PortfolioItemRequests", :type => [:request, :topology, :v1] do
  ...
  context "when adding portfolio items" do
    let(:add_to_portfolio_svc) { instance_double(V1x0::ServiceOffering::AddToPortfolioItem) }
    ...
    before do
      allow(V1x0::ServiceOffering::AddToPortfolioItem).to receive(:new).with(permitted_params).and_return(add_to_portfolio_svc)
    end
