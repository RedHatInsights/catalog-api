describe "IconRequests", :type => :request do
  context "icons" do
    let(:icon_id) { 1 }
    ...
    context "when we have to hit topology for the icon data" do
      ...
      it "invalid portfolio id" do
        get "#{api}/portfolio_items/12345678/icon", :headers => default_headers
        expect(response).to have_http_status(404)
      end

      it "raises ArgumentError" do
        expect(api_instance).to receive(:show_service_offering_icon).with(topology_service_offering_icon.id.to_s).and_raise(ArgumentError)
